class Solution {
    ...
    // productExceptSelf(nums, size=nums.length, left=1, right=1) {
    //     return nums.reduce((prod, curr, i) => {
    //         [left, prod[i]] = [left*curr, left]
    //         return prod
    //     }, Array(size)).reduceRight((prod, curr, i) => {
    //        [right, prod[i]] = [ right*= i<size-1? nums[i+1] : 1, curr*right]
    //         return prod;
    //     }, Array(size));
    // }
    productExceptSelf(nums) {
