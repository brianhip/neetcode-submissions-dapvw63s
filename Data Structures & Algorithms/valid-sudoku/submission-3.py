class Solution:
    def isValidSudoku(self, board: List[List[str]]) -> bool:
        ...
        ROWS, COLS, BOXS = self.buildRowsColsBoxes(board)
        return self.validation(ROWS, COLS, BOXS)
        

    def buildRowsColsBoxes(self, board: List[List[str]]) -> List[List]:
    ...
    def validation(self, ROWS, COLS, BOXS) -> bool:
        for i in range(1, 10):
            number = str(i)
            for j in range(9):
